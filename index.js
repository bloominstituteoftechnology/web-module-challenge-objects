...
createMenuItem("cookies", 5, "dessert");
createMenuItem("smoothie", 7, "drinks");
createMenuItem("fries", 3, "sides");
...
burger.discount = (type) => {
  type = type.toLowerCase();
  if (type === "student" || type === "teacher") {
    return burger.price*.75;
  }
  else return burger.price*.9;
}
...
function getReviewByRating(arr, rating) {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].rating < rating+1 && arr[i].rating >= rating) {
...
/** STRETCH 2: Write a function called 'getLongReview' that returns an array containing all reviews longer than 15 words. 
...
*/
  function getLongReviews(arr) {
    let longReviews = [];
    for (let i = 0; i < arr.length; i++) {
      let feedback = arr[i].feedback.split(" ");
...
let nissan = carMaker(5000)
console.log(nissan.drive(300));
