...
var divJamaria = {
  ...
  rating: 5.0,
  feedback: "I'm a god!",
  fictitious: "I'm a king!",
};
reviews.push(divJamaria);
console.log(reviews);
/* 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 5: 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 
...
function getReviewByIndex(arrayOfObjects, desiredIndex) {
  var answer =
    arrayOfObjects[desiredIndex].name +
    " gave the restaurant a " +
    arrayOfObjects[desiredIndex].rating +
    " star review, and their feedback was: " +
    arrayOfObjects[desiredIndex].feedback;
  return answer;
}
...
*/
function getLastReview(receivedArray) {
  var lastIndex = receivedArray.length - 1;
  return `${receivedArray[lastIndex].name} gave the restaurant a ${receivedArray[lastIndex].rating} star review, and their feedback was: ${receivedArray[lastIndex].feedback}`;
}
...
function getReviewByRating(allReviews = reviews, arrayRating = 4) {
  // looping through all the items in the array
  // check to see if rating is between the range  == arrayRating && i < a;)
  var a = arrayRating + 1;
  var filteredArray = [];
  for (var i = 0; i < allReviews.length; i++) {
    if (allReviews[i].rating >= arrayRating && allReviews[i].rating < a) {
      filteredArray.push(allReviews[i]);
    }
  }
...
function getLongReviews(allReviews = reviews) {
  var newList = [];
  for (var i = 0; i < allReviews.length; i++) {
    var x = allReviews[i].feedback;
    // console.log(x)
    var y = x.split(" ");
    // console.log(y)
    if (y.length > 15) {
      newList.push(allReviews[i]);
    }
  }
  // console.log(newList)
  return newList;
...
function carMaker(odometer = 10) {
  const car1 = {
    odometer: 10,
    drive: function addDistance(distance) {
      this.odometer = odometer + distance;
    },
  };
  return car1.drive(100), console.log(car1.odometer);
}
carMaker();
