...
console.log(createMenuItem('pasta',9,'lunch/dinner'));
console.log(createMenuItem('lasagna',12,'dinner'));
...
function addReview(arr, user, stars, review){
  arr.push({name: user, rating: stars, feedback: review});
  return arr;
}
// addReview(reviews, 'Kade', 5, 'Just perfect');
...
function getReviewByIndex(arr, num) {
  console.log(`${arr[num].name} gave the restaurant a ${arr[num].rating} star review, and their feedback was: ${arr[num].feedback}`);
...
*/
 ...
 function getReviewByRating(arr,stars) {
   const newArray = [];
    for(let i=0; i < arr.length; i++){
      if(arr[i].rating >= stars && arr[i].rating < stars + 1){
        newArray.push(arr[i]);
      }
    }
    return newArray;
...
function carMaker(odom) {
  const car = {
    ...
    drive: function (miles){
      let newMiles = this.Odometer + miles;
      return newMiles;
    }
  }
  // return car;
