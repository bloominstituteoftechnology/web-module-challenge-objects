...
// createMenuItem(Ruben, 7, lunch);
// createMenuItem(Salad, 4, lunch);
/* 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀 Task 2: 🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
...
export const burger = {
	...
	price: 18,
	category: "Lunch",
	discount: function (tier) {
		if (tier == "teacher" || tier == "student") {
			let numVal1 = this.price;
			let numVal2 = 25 / 100;
...
function getReviewFromName(array, target) {
...
getReviewFromName(reviews, "Julius");
...
console.log(addReview(reviews, "Stan", 6, "Love the nachos!!"));
...
function editFeedback(array, name, feedback) {
	let index = array.findIndex((objectEntry) => objectEntry.name === name);
	let objectToEdit = array[index];
	objectToEdit.feedback = feedback;
	return array;
}
...
function getReviewByRating(array, score) {
	let targetReviews = array.filter(
		(objectEntry) =>
			objectEntry.rating >= score && objectEntry.rating < score + 0.9
	);
	console.log(targetReviews);
}
...
function getLongReviews(array) {
	return array[1].feedback.split(" ").length;
}
