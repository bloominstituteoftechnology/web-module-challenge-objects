...
console.log(createMenuItem('burrito', 10, 'Mexican'))
console.log(createMenuItem('Sushi', 20, 'Japanese'))
console.log(createMenuItem('Linguini', 22, 'Italian'))
...
export const burger = {
  ...
  discount: function (person) {
    if (person === 'teacher' || person === 'student') {
      const discountDifference = this.price * .25;
      return this.price - discountDifference;
    }
    else {
      const discountDifference = this.price * .10;
      return this.price - discountDifference;
    }
  }
}
...
// reviews.push({ name: 'Juan', rating: 3, feedback: 'This place was okay I guess...' })
console.log(reviews)
...
function getLongReviews(arr) {
  const result = []
  for (let item of arr) {
    if (item.feedback.split(' ').length > 15) {
      result.push(item)
    }
  }
  return result;
